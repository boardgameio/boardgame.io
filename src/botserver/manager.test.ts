...
describe('Bot manager', () => {
  ...
  it('adds single bot to game', () => {
    ...
    expect(mockClientImpl.start).not.toHaveBeenCalled();
    expect(mockClientImpl.subscribe).not.toHaveBeenCalled();
    botManager.addBotToGame(gameName, 'testMatchId', [
      { playerID: 'p1', playerCredentials: 'pc1' },
    ]);
    expect(mockClient).toHaveBeenCalled();
    expect(mockClientImpl.start).toHaveBeenCalled();
    expect(mockClientImpl.subscribe).toHaveBeenCalled();
  });
  it('adds multiple bots to game', () => {
    ...
    expect(mockClient).toHaveBeenCalled();
    expect(mockClientImpl.start).toHaveBeenCalledTimes(3);
    expect(mockClientImpl.subscribe).toHaveBeenCalledTimes(3);
  });
  it('calls runBot when its bots turn', async () => {
    ...
    expect(runBot).toHaveBeenCalledWith(state);
    expect(clientP2.moves[moveName]).toHaveBeenCalledWith(moveArgs);
  });
});
